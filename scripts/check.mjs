...
for(const l of ['en','ko']){const p=l==='ko'?'ko/':'';const pub=fs.readFileSync(`${root}/${p}publications/index.html`,'utf8');const n=(pub.match(/data-publication /g)||[]).length;if(n!==127)errors.push(`${l}: expected 127 publications, found ${n}`);const members=JSON.parse(fs.readFileSync(`data/${l}/members.json`));if(members.categories.reduce((s,c)=>s+c.members.length,0)!==37)errors.push(`${l}: roster count`);const teaching=JSON.parse(fs.readFileSync(`data/${l}/teaching.json`));if(teaching.semesters[0].courses.length!==2)errors.push(`${l}: current course count`);}
if(errors.length){console.error([...new Set(errors)].join('\n'));process.exit(1)}console.log(`Checked ${pages.length} pages: local links, images, metadata, 127 publications, 37 members and two current courses per language.`);
